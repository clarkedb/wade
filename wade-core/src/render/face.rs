//! Wade's face: two eyes shaped by cutting lids out of them, and an accent
//! beside them, driven entirely by `Pose` (docs/character.md#appearance).
...
/// How far the eyes move at full gaze, per axis.
...
impl Face {
    fn new(pose: &Pose) -> Face {
        let (w, h) = pose.eye_size;
        let x = coord(layout::WADE_CENTER.x) + pose.gaze.0 * GAZE_RANGE.0 + pose.face_offset.0;
        let y = coord(layout::WADE_CENTER.y) + pose.gaze.1 * GAZE_RANGE.1 + pose.face_offset.1;
...
{
    let face = Face::new(pose);
    draw_eye(pose, face.left, target)?;
    draw_eye(pose, face.right, target)?;
    draw_accent(pose, &face, target)
}

fn draw_eye<D>(pose: &Pose, eye: Eye, target: &mut D) -> Result<(), D::Error>
where
    D: DrawTarget<Color = Rgb565>,
{
    let (w, h) = eye.size;
